...
import 'package:appointments/helpers/consumable_store.dart';
...
const String _kConsumableId = 'gain_popularity';
const String _kUpgradeId = 'gain_popularity_2';
const String _kSilverSubscriptionId = 'business_user_sub';
const String _kGoldSubscriptionId = 'business_user_sub';
...
class PremiumAccountManagement extends StatefulWidget {
  ...
  State<PremiumAccountManagement> createState() =>
      PremiumAccountManagementState();
}

class PremiumAccountManagementState extends State<PremiumAccountManagement> {
  ...
  Future<void> initStoreInfo() async {
    try {
      ...
      // ignore: avoid_print
      print('Error querying product details: $error');
        ...
        if (purchaseDetails.status == PurchaseStatus.error) {
          ...
          if (valid) {
            if (purchaseDetails.productID == _kSilverSubscriptionId) {
              // User successfully subscribed to silver subscription
              // Save subscription information to Realtime Database
              saveSubscriptionToDatabase(purchaseDetails);
            }
            unawaited(deliverProduct(purchaseDetails));
            // Check if the user has a valid subscription
            if (purchaseDetails.productID == _kSilverSubscriptionId ||
                purchaseDetails.productID == _kGoldSubscriptionId) {
  ...
  void _checkSubscriptionStatus() {
    bool hasValidSubscription = _purchases.any((purchase) =>
        purchase.productID == _kSilverSubscriptionId ||
        purchase.productID == _kGoldSubscriptionId);

    if (hasValidSubscription) {
      // User has a valid subscription, handle accordingly
      // ignore: avoid_print
  ...
  void saveSubscriptionToDatabase(PurchaseDetails purchase) {
    ...
    if (user != null) {
      ...
      // Create a unique key for the subscription entry
      String subscriptionKey =
          databaseReference.child('subscriptions').push().key!;
      ...
      databaseReference
          .child('subscriptions')
          .child(subscriptionKey)
          .set(subscriptionData)
          .then((value) {})
          .catchError((error) {
        // ignore: avoid_print
        print('Error saving subscription data: $error');
      });
    }
  }
